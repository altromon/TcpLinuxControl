		   ...
		   int optlen)
{
	...
	case TCPI_STATE:
		...
		if(val!=-1)		//Si el campo esta a -1 es porque no hay que modificarlo
		{
			sk->state = (u_int8_t)val;
		}
		break;
	...
	case TCPI_LAST_DATA_RECV:
		...
		if(val!=-1)
		{
			tp->ack.lrcvtime = now + (val*HZ)/1000;
		}
		break;
